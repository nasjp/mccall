...
- [ ] 5. Tauri commandsとイベントシステムの実装
  ...
  - [ ] 5.3 現在状態取得コマンドの実装（未着手）
    - get_timer_state で実行中状態/残り時間/チェックイン待ち/セッション情報を返す
    - アプリ再起動時のUI復元に使用
    - _要件: 8.1, 8.2_

- [ ] 6. React フロントエンドの基本構造実装（未完了）
- [x] 6.1 アプリケーション状態管理を実装（Done）
  ...
  - [ ] 6.3 画面遷移/導線（Timer/Editor/Stats）を実装（未着手）
...
- [ ] 11. 統計・ログ機能の実装
  - [ ] 11.1 セッション統計の計算を実装（未完了）
    - 作業時間・休憩時間の集計機能を実装
    - Check-in skip率の計算機能を実装
    - 不足: セッション保存が未実装のため集計が空になる
    - _要件: 5.2, 5.3, 5.4_
